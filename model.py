...
def phi(U, phi_params):
    return phi_params['r_max']/(1+phi_params['k']*np.exp(phi_params['beta']*(1-U/phi_params['thresh'])))

def phi_prime(U, phi_params):
    exp_term = np.exp(phi_params['beta']*(1-U/phi_params['thresh']))
...
def urb_senn_rhs(y, t, t_post_spike, g_E_D, syn_pots_sum, I_ext, neuron):
    ...
    dy[1] = -neuron['g_L']*y[1] + neuron['g_S']*(y[0]-y[1]) + g_E_D*(neuron['E_E']-y[1])
